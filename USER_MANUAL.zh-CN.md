...
agent-skills 是由 Addy Osmani 创建的 AI 智能体专业技能集合，旨在增强代理能力和协作效率。该项目提供了一系列经过精心设计的技能，帮助 AI 代理更好地理解和执行各种开发任务。

#### 4.4.2 核心功能

- **专业技能集合**：提供多种专业领域的技能，涵盖开发、设计、测试等多个方面
- **增强代理能力**：帮助 AI 代理更好地理解上下文和执行复杂任务
- **协作效率提升**：通过标准化的技能接口，提高代理之间的协作效率
- **模块化设计**：技能采用模块化设计，易于扩展和定制

#### 4.4.3 安装与配置

安装后会自动复制到 Claude 的技能目录，无需额外配置即可使用。

#### 4.4.4 技能列表

- **代码理解**：帮助代理更好地理解代码结构和逻辑
- **设计模式识别**：识别和应用常见的设计模式
- **性能优化**：提供性能分析和优化建议
- **安全审查**：识别潜在的安全问题和漏洞
- **文档生成**：自动生成代码文档和注释
...
agent-skills 会根据上下文自动激活，增强代理在处理相关任务时的能力。无需手动激活，代理会在需要时自动使用相应的技能。

#### 4.4.6 在软件开发中的价值

- **增强代理能力**：使 AI 代理能够处理更复杂的开发任务
- **提高代码质量**：通过专业技能指导，提高代码的质量和可维护性
- **加速开发过程**：减少代理在理解和处理任务时的时间
- **知识标准化**：提供标准化的专业知识和最佳实践

### 4.5 understand-anything

#### 4.5.1 项目概述

understand-anything 是一个 Claude Code 插件，能够将任何代码库、知识库或文档转换为交互式知识图谱。它使用多代理分析管道扫描项目，提取每个文件、函数、类和依赖关系，然后构建一个可探索、搜索和问答的交互式仪表板。

#### 4.5.2 核心功能

- **代码结构映射**：将代码库解析为知识图谱，每个文件、函数和类都是可点击、搜索和探索的节点
- **交互式仪表板**：提供可视化界面，支持缩放、搜索和探索
- **业务逻辑分析**：切换到领域视图，查看代码如何映射到实际业务流程
- **知识库分析**：分析 Karpathy-pattern LLM wiki，生成带有社区聚类的知识图谱
- **引导式学习路径**：自动生成的架构演示，按依赖关系排序
- **模糊和语义搜索**：通过名称或含义查找任何内容
- **差异影响分析**：查看更改对系统其他部分的影响
- **多语言支持**：支持中文、英文、日文、韩文、俄文等多种语言

#### 4.5.3 安装与配置

安装后会自动运行其安装脚本，配置 Claude Code 插件。也可以手动安装：
...
- **/understand-chat**：询问代码相关问题
- **/understand-diff**：分析当前更改的影响
- **/understand-explain**：深入了解特定文件或函数
- **/understand-onboard**：生成新团队成员的入职指南
...
1. **分析代码库**：
   ...
   /understand
   ```
   多代理管道扫描项目，提取每个文件、函数、类和依赖关系，构建知识图谱。

2. **探索仪表板**：
   ```
   /understand-dashboard
   ```
   打开交互式 Web 仪表板，可视化代码库为图谱。
...
- **快速理解新代码库**：新团队成员可以快速理解 200,000 行代码的项目
- **可视化架构**：看到代码的整体结构，理解各个部分如何组合
- **影响分析**：在提交前了解更改的影响范围
- **知识共享**：将图谱提交到 Git，团队成员可以共享理解
- **持续学习**：通过引导式学习路径，按正确顺序学习代码库

### 4.6 claude-for-legal

#### 4.6.1 项目概述

claude-for-legal 是 Anthropic 官方发布的法律工作流插件集，为法律专业人士提供全面的 AI 辅助支持。该项目覆盖公司法务、隐私、雇佣、监管、知识产权等多个法律领域，通过智能代理和技能实现法律工作流程的自动化。

#### 4.6.2 核心功能

- **商业法律**：合同审查、NDA分类、修订追踪、续约监控
- **公司法律**：尽职调查、董事会决议起草、实体合规、关闭清单
- **雇佣法律**：解雇审查、入职审查、员工分类、请假追踪
- **隐私法律**：DSAR响应、DPA审查、隐私影响评估（PIA）、政策监控
- **产品法律**：发布审查、营销声明检查、问题分类
- **监管法律**：监管动态跟踪、政策差异分析、差距追踪
- **知识产权**：专利、商标、版权相关法律支持
- **AI治理**：AI使用政策合规、风险管理
...
| 插件名称 | 功能描述 |
|---------|---------|
| commercial-legal | 商业合同审查、供应商协议审查、NDA分类 |
| corporate-legal | 尽职调查、董事会决议、实体合规管理 |
| employment-legal | 雇佣审查、解雇风险评估、员工分类 |
| privacy-legal | DSAR响应、DPA审查、隐私影响评估 |
| product-legal | 产品发布审查、营销声明验证 |
| regulatory-legal | 监管动态监控、政策差异分析 |
| litigation-legal | 诉讼支持、案件管理 |
| ip-legal | 知识产权管理、专利商标支持 |
| ai-governance-legal | AI治理、合规审查 |
...
2. **安装特定插件**：
   ```
   /plugin install commercial-legal
   /plugin install corporate-legal
   /plugin install privacy-legal
   ```

3. **使用法律技能**：
   ```
   /commercial-legal:review       # 审查合同
   /commercial-legal:nda-triage  # NDA分类
   /corporate-legal:diligence-issue-extraction  # 尽职调查问题提取
   /privacy-legal:dsar-response   # DSAR响应
   /employment-legal:termination-review  # 解雇审查
   ```
...
> ⚠️ **免责声明**：这些插件的所有输出均为律师审查的草案，不是法律建议，不是法律结论，不能替代律师。所有输出必须经过律师审核、验证并承担专业责任后方可使用。

#### 4.6.7 在法律工作中的价值

- **提高效率**：自动化繁琐的法律文档审查工作
- **风险识别**：快速识别合同风险和合规问题
- **时间管理**：追踪重要的法律截止日期（续约、监管等）
- **知识管理**：系统化管理法律政策和流程
- **持续监控**：自动监控合同状态和监管动态
...
ponytail 是一个为 AI 代理设计的最小化代码编写技能，基于阶梯式决策规则，让 AI 只编写必要的代码。它的核心理念是"只写任务需要的代码"，而不是追求"最少的 token"，同时保持安全性、可维护性和可访问性。

#### 4.7.2 核心功能

- **阶梯式决策规则**：在编写代码前，按照阶梯顺序判断最佳实现方式
  1. 这个功能是否需要存在？→ 不需要：跳过（YAGNI）
  2. 代码库中已经有了？→ 复用，不重写
  3. 标准库支持？→ 使用标准库
  4. 平台原生功能？→ 使用原生功能
  5. 已安装的依赖？→ 使用依赖
  6. 一行代码？→ 用一行代码
  7. 只有在以上都不适用时：编写最小可行代码

- **模式切换**：支持 lite、full、ultra 三种模式
  - lite：轻量级模式，适用于简单任务
  - full：完整模式，适用于大多数任务（默认）
  - ultra：极端模式，适用于代码库存在严重问题时

- **安全保障**：信任边界验证、数据丢失处理、安全性和可访问性永不会被削减
...
# 或通过配置文件 ~/.config/ponytail/config.json
# { "defaultMode": "full" }
```
...
| /ponytail-review | 代码审查，识别冗余和过度构建 |
| /ponytail-audit | 安全审计 |
| /ponytail-debt | 技术债务分析 |
| /ponytail-gain | 衡量影响评分板 |
| /ponytail-help | 显示帮助信息 |

#### 4.7.5 使用方法

1. **安装后自动激活**：ponytail 在每个会话中自动激活，无需手动操作
...
- **减少代码冗余**：避免 AI 过度构建，只写必要的代码
- **提高效率**：更快的开发速度，更低的成本
- **保持质量**：在减少代码的同时保持安全性和可维护性
- **标准化实践**：统一的编码决策规则，提高团队协作效率
